...
def buildFreqDic(fn):
	corpora = [nltk.corpus.brown, nltk. corpus.gutenberg, nltk.corpus.reuters, 
				nltk.corpus.webtext]
	fr_d = {}
...
def build_dic():
	...
	for wrd, freq in fr_d.items():
		...
		if wrd not in all_d:
			pos = get_pos(wrd)
			if pos in ['CC', 'DT', 'EX', 'IN', 'MD', 'PDT', 'PRP', 'PRP$', 'RP', 'TO', 'UH', 'WDT', 'WP', 'WRB']:
				ok = True
...
def my_lemmatize_fromscratch(wrd, pos=None):
	#get_synset = wordnet.synset_from_pos_and_offset
	ssdb = wordnet._lemma_pos_offset_map#for each word gives dictionary like:
	#{'a': [3478, 754289],...} Where for each pos you get a list of synsets (their ids)
	if pos is None:
		pos = 'nvasr'
	resD = {}
	for p in pos:
		resD[p] = set()
		for form in wordnet._morphy(wrd, p): 
			#that's what wordnet.synsets uses, which results in "beses" being considered a
			#form of 'be'
			if p in ssdb[form]: 
				if 1: #without this everything will be lowercase 
					found = False
					get_synset = wordnet.synset_from_pos_and_offset
					for ssid in ssdb[form][p]:
						ss = get_synset(p, ssid)
						for lemma in ss.lemma_names():
							if lemma.lower() == form:
								resD[p].add(lemma)#for example wa becomes WA
								#found = True
								#break
						#if found: break
				#resD[p].add(form)
...
def build_lemmatization_dic(fn):
	...
	with open(fn, 'w') as f:
		for r in res_l:
			lemstr = '; '.join([f"{pos}. {', '.join(lemset)}" for pos, lemset in r[1].items() if lemset])
...
def scrapewiktionary(fn_save, fn_wordlist=None):
	...
	with open(fn_cur, 'w') as fc: pass 
	with open(fn_save, 'w') as fc: pass 
	with open(fn_failed, 'w') as fc: pass 
	...
	for wrd in fr_l:
		...
		if skip:
			if not wcur or wrd == wcur: skip = False
			else: 
				print('Skipping.')
				continue
...
def process_wikdict(fn, fnnew):
	...
	for k, v in wdic.items():
		...
		sepsenseL = []
		for sepsense_d in v:
			if "definitions" not in sepsense_d: continue
			posL = []
...
def find_unhelpful_defs(fn_defs, fnnew, fn_tolookup):
	...
	for w, defL in defD.items():
		if len(defL) > 1: continue
		posL = defL[0]
		if len(posL) > 1: continue

		dfns = posL[0]['defs']
		if len(dfns) > 1: continue
		dfn = dfns[0]
		# Of or pertaining to
		#In a...Manner,  Like a , One who, That which , capable of being , degree of being, able to be, 
		# The ability to be  
		soiL = ['lural of ', 've form of ', 'participle of ','spelling of ', 'singular of ',
		'Synonym of ', 'bsolete form of ', 'istoric form of ']
		otherL = ['elating to ', ' tate of being ','uality of being ','uality of not being ' ]
	...
	print('Done.', len(defD), 'total definitions examined.', len(betterformsD), 'better spellings found.',
	len(betterformsL), 'words to look up.')
...
class def2refdata:
		...
		#In a...Manner,  Like a , One who, That which , capable of being , degree of being, able to be, 
		# The ability to be  
		formOfL = ['Plural of ', 'Indicative form of ', 'Alternative form of ', 'Comparative form of ',
		'Superlative form of ', 'participle of ','spelling of ', 'singular of ',
		'Obsolete form of ', 'Historic form of ']
		otherL = ['Synonym of ', 'Relating to ', 'State of being ','Quality of being ',
		'Quality of not being ' ]
		moreL = ['#Of or pertaining to ', '#Pertaining to ', '#Like ', '#One who ', '#That which ', '#Capable of being ',
		'#Degree of being ', '#Able to be ', '#Ability to be ', '#The ability to be ']
		...
		['sy','re','be','be','nb', 'of', 'of','li', '1w', 'tw', 'ab', 'de', 'ab', 'bb', 'bb']
		altform_refs = ['pl','tm', 'af', 'co', 'su', 'tm', 'sp', 'si']
		altform_refs += [x.upper() for x in altform_refs]

		terminators = list(')(,;:[.') + ['See ', ' -']
		endingpairs = [('city', 'ciousness'), ('ity','ness'), ('ty','ness'), 
		('ous','ic'), ('ical','ic'), ('al','ic'), ('istic','ic'), ('ar','tic'), 
		('ist','er'), ('ism','age'), ('ist','ian'), ('al','ic'), ('ar','atory'), 
		('ism','ianism'), ('ly','like'), ('y','ie'), ('ism','y'), ('gue','gist'), 
		('ory','ive'), ('ish', 'y'), ('ing','al'), ('y','esis'), ('ary','al'), ('ary','ist'), 
		('ary','ine'), ('y','ing'), ('ate','ar'), ('ated','ar'), ('try','cy'), 
		('ing','ion'), ('ing','ent'), ('ing',''), ('ist',''), ('tus','t'), ('tsa','na'),
		('ose','al'), ('ose','ar'), ('ing','ation'), ('ingly','ingly'), ('ly','ingly'), ('um', '')]
...
def def2ref(wrd, dfn, defD):
	...
	if not ref:
		...
		ref = 'SI' if m else 'SY'
		if 0:
			print(wrd, 'dfn:', dfn, 'orig:',orig,m, mm, 'eddist:', dfn2, wrd2, nltk.edit_distance(dfn2, wrd2),
			'ref:', ref)
			input()
		return ref, dfn
...
def build_wikdefs_withrefs(fn_defs, fnnew):
		...
		for posL in sepsenseL:
			for pos in posL:
				...
				if not norefs:
					pos['refs'] = refL
					import random
					if (pos['refs'] and pos['refs'][0][0]!= 'PL' and random.random() < -0.001):# and wrd
						print(pos)
						input()
	...
	print('Done.', len(defD), 'total definitions examined.', len(betterformsL), 'references found.',
	len(tolookupL), 'words to look up.')
...
def build_good_wikdefs(fn_withrefs, fnnew, fnwordlist):
	...
	for w, sepsenseL in defD.items():
		...
		for posL in sepsenseL:
			for pos in posL:
				if 'defs' not in pos: continue
				if pos['pos'].lower() =='proper noun': continue
				if 'refs' not in pos: 
					good = True
					continue
				dfns = pos['defs']
